...
function ProfileCard({ title, subtitle, image, size = 'small', tab, className }: ProfileProps) {
    ...
    return (
        <PressableScale
            className={cn(isLarge ? 'relative overflow-hidden flex-1 bg-background rounded-[32px] p-5 gap-1' : 'relative overflow-hidden w-40 h-36 bg-background rounded-[32px] p-5', className)}
            onPress={handlePress}
        >
            ...
            {image && (
                <Image
                    ...
                    className={
                        isLarge
                            ? 'absolute -bottom-20 -right-6 h-44 w-44'
                            : 'absolute -bottom-10 -right-3 h-32 w-32'
                    }
                />
            )}
        </PressableScale>
