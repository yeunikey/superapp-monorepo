    ...
    return (
        <View className='flex flex-row justify-between items-center bg-white p-6 pt-3 rounded-b-[32px]'>
            <View className='flex flex-row gap-4 items-center'>
                ...
                {loggedUser && <Avatar user={loggedUser} />}

                <View className='flex flex-col gap-0.5'>
                    <Text className='text-lg font-semibold'>{loggedUser?.name} {loggedUser?.surname[0] + '.'}</Text>
                    <View className='flex flex-row gap-1 items-center'>
                        <Text className='font-semibold text-secondary'>{loggedUser?.scores}</Text>
